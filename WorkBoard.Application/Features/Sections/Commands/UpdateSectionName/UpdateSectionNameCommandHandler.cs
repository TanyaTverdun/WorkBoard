...
public class UpdateSectionNameCommandHandler 
    : IRequestHandler<UpdateSectionNameCommand, Unit>
{
    ...
    public async Task<Unit> Handle(
        UpdateSectionNameCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (membership == null || membership.UserRole == BoardRole.Observer)
        {
            throw new ForbiddenAccessException(
                "You do not have permission to modify sections on this board.");
        }
        ...
        try
        {
            await uow.SectionRepository.UpdateAsync(
                section, 
                cancellationToken);
